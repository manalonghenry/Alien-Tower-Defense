...
void RenderSystem::initializeFonts() {
	...
	// Initialize FreeType and load fonts (unchanged)
	FT_Library ft;
	...
	FT_Face face;
	std::string path = std::string(PROJECT_SOURCE_DIR) + "/data/fonts/Kenney_Mini.ttf";
	...
	for (unsigned char c = 0; c < 128; c++) {
		...
		Entity char_entity = Entity();
		Character& character = registry.characters.emplace(char_entity);
		...
		registry.character_map[c] = character;
	}
...
void RenderSystem::initializeGlGeometryBuffers()
{
	...
	glGenBuffers(1, &vertex_buffers[(GLuint)GEOMETRY_BUFFER_ID::TEXT]);
	glBindBuffer(GL_ARRAY_BUFFER, vertex_buffers[(GLuint)GEOMETRY_BUFFER_ID::TEXT]);
	// Allocate an initial buffer with dynamic draw (6 vertices per glyph, each vertex has 4 floats)
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 6 * 4 * 100, nullptr, GL_DYNAMIC_DRAW); // 100 glyphs as an example
	...
	// ADDED
	std::vector<ColoredVertex> quad_vertices = {
		{ { -0.5f, -0.5f, 0.0f }, { 1, 1, 1 } },
		{ {  0.5f, -0.5f, 0.0f }, { 1, 1, 1 } },
		{ {  0.5f,  0.5f, 0.0f }, { 1, 1, 1 } },
		{ { -0.5f,  0.5f, 0.0f }, { 1, 1, 1 } }
	};
	std::vector<uint16_t> quad_indices = { 0, 1, 2, 0, 2, 3 };
	bindVBOandIBO(GEOMETRY_BUFFER_ID::FILLED_QUAD, quad_vertices, quad_indices);
